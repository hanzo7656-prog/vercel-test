...
# ============================================================
# مدیریت PostgreSQL - نسخه ۲.۲ (اصلاح شده با Self-Healing)
# ============================================================
...
class PostgreSQLManager(DatabaseBase):
    """مدیریت PostgreSQL با Self-Healing و کش ورژن"""
    ...
    def connect(self) -> bool:
        """برقراری اتصال به PostgreSQL با تنظیمات خودکار"""
        try:
            ...
            logger.error(f"❌ خطای عملیاتی PostgreSQL ({self.name}): {e}")
    ...
    def is_connected(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"⚠️ PostgreSQL ({self.name}) connection lost: {e}")
            self._connected = False
            self._cursor = None
            return False
    ...
    def ensure_connection(self) -> bool:
        ...
        logger.warning(f"⚠️ PostgreSQL ({self.name}) متصل نیست، تلاش برای reconnect...")
        ...
        for attempt in range(self._max_reconnect_attempts):
            if self.connect():
                logger.info(f"✅ PostgreSQL ({self.name}) reconnect successful (attempt {attempt + 1})")
                return True
            time.sleep(2 ** attempt)  # exponential backoff
        
        logger.error(f"❌ PostgreSQL ({self.name}) reconnect failed after {self._max_reconnect_attempts} attempts")
        return False
    ...
    def execute(self, query: str, params: tuple = None) -> List[Dict]:
        ...
        if not self.ensure_connection():
            logger.error(f"❌ PostgreSQL ({self.name}) اتصال برقرار نیست")
            return []
        ...
        try:
            ...
            logger.warning(f"⚠️ PostgreSQL ({self.name}) connection error: {e}")
            self._connected = False
            self._cursor = None
            
            if self.connect():
                logger.info(f"✅ PostgreSQL ({self.name}) reconnected, retrying query...")
                try:
                    self._cursor = self._connection.cursor()
                    self._cursor.execute(query, params or ())
                    if query.strip().upper().startswith("SELECT"):
                    ...
                    logger.error(f"❌ PostgreSQL ({self.name}) retry failed: {retry_error}")
                    if self._connection:
                        self._connection.rollback()
                    return []
            else:
                logger.error(f"❌ PostgreSQL ({self.name}) reconnect failed")
                return []
                
        except Exception as e:
            logger.error(f"❌ PostgreSQL ({self.name}) query error: {e}")
            if self._connection:
                try:
                    self._connection.rollback()
                except:
    ...
    def ping(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"⚠️ PostgreSQL ({self.name}) ping failed: {e}")
            self._connected = False
            self._cursor = None
            return False
    
    def health_check(self) -> Dict[str, Any]:
        """بررسی سلامت کامل PostgreSQL"""
