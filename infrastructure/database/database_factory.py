...
# ============================================================
# کارخانه ساخت دیتابیس‌ها - نسخه ۳.۱
...
class DatabaseFactory:
    ...
    def __init__(self) -> None:
        ...
        self._health_check_interval: int = 120   # 🆕 از ۳۰ به ۱۲۰
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            self._health_check_interval = max(
                settings.get("health_check_interval", 120),
                120
            )
    ...
    def _connect_all_with_retry(self) -> None:
        ...
        for db_name, db_config in databases.items():
            ...
            logger.info(
                f"  → Initializing '{db_name}' "
                f"(type={db_config.get('type', 'unknown')})..."
            )
            started = time.time()
            ...
            if success:
                logger.info(f"✅ Database '{db_name}' registered in {elapsed:.2f}s")
            else:
                logger.error(
                    f"❌ Database '{db_name}' failed after "
                    f"{self._max_retries} attempts in {elapsed:.2f}s"
                )
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if db_instance.connect():
                    ...
                    logger.info(
                        f"✅ Database '{db_name}' registered (roles: {roles})"
                    )
                    return True
                else:
                    # 🆕 register حتی اگر connect نشد
                    registry.register(db_name, db_instance, roles)
                    logger.warning(
                        f"⚠️ Database '{db_name}' registered but NOT connected "
                        f"(roles: {roles})"
                    )
                    return False
